	...
	if(q_interp_method(1) == 1) then	! jw
		...
		do i=1,num_SS_cell
			do t=2,q_data_num(SS_Q_ser_id(i))
				...
				! jw
				! jw
				u1 = q_ser_time(t-1,SS_Q_ser_id(i)) - reference_diff_days * 86400.0	! jw
				u3 = q_ser_time(t  ,SS_Q_ser_id(i)) - reference_diff_days * 86400.0	! jw
								
				if(u2 >= u1 .and. u2 <= u3) then
					v1 = q_ser_Q(t-1,SS_Q_ser_id(i))		! jw
					v3 = q_ser_Q(t  ,SS_Q_ser_id(i))		! jw
