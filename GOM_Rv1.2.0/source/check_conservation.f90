...
subroutine check_conservation
   ...
   do i = 1, maxele
      do k = bottom_layer_at_element(i), top_layer_at_element(i) ! wet elements
         ...
         u_average=0 ! average u
         v_average=0 ! average u
         ...
         w_average=(wn_cell(k,i)+wn_cell(k-1,i))/2 ! we has been extended
   ...
   do i=1,maxele
      ...
      do l=1,tri_or_quad(i)
         ...
         if(adj_cellnum_at_cell(l,i)==0) then ! bnd side
            do k=bottom_layer_at_face(i_face_no),top_layer_at_face(i_face_no) ! wet sides
               fluxbnd=fluxbnd+un_face(k,i_face_no)*face_length(i_face_no)*dz_face(k,i_face_no)
            end do
         else if(adv_depth2(adj_cellnum_at_cell(l,i))==0) then ! channel side
            do k=bottom_layer_at_face(i_face_no),top_layer_at_face(i_face_no) ! wet sides
