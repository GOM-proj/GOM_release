...
subroutine coordinate_conversion(x1,y1,UTM_PROJECTION_ZONE,option, x2,y2)
	...
	real(dp), parameter :: scfa = 0.9996_dp ! scale factor
	...
	! real(dp):: rx_save,ry_save,rlon_save,rlat_save
	! End of local variables ==================================================!
	
	! Coordinate transformation ===============================================!
	e2 = 1-(semi_minor_axis/semi_major_axis)**2 ! 1-(b/a)^2
	e4 = e2*e2
	e6 = e2*e4
	ep2=e2/(1.d0-e2)
   ...
   if(option == 1) then ! Long/Lat -> UTM
		dlon = x1 ! dlong: degree longitude
		dlat = y1 ! dlat: degree latitude
   else if(option == 2) then ! UTM -> Long/Lat
   	xx = x1
     	yy = y1
     	if(lsouth) then
     		yy = yy - 1.d7
   ...
   if(option == 1) then ! Long/Lat -> UTM
      rlon = deg2rad*dlon ! rlon: radian longitude
      rlat = deg2rad*dlat ! rlat: radian latitude
   ...
   ! UTM to Lat/Lon conversion ===============================================@
  	...
  	if(option == 1) then ! lon/lat -> UTM
   	x2 = xx
   	if(lsouth) then
   		yy = yy + 1.d7
   	end if
    	y2 = yy
	else if(option == 2) then ! UTM -> lon/lat
   	x2 = dlon
    	y2 = dlat
   endif	
