...
subroutine open_tser_out_files
   ...
   ! End of local variables ==================================================!
	...
	! define time conversion factor (for global variable)
	! julian_day to printing time
	if(tser_time == 1) then	! second
		tser_time_conv = 86400.0
	else if(tser_time == 2) then ! minute
		tser_time_conv = 1440.0
	else if(tser_time == 3) then ! hour
		tser_time_conv = 24.0
	else if(tser_time == 4) then ! day
		tser_time_conv = 1.0
	end if

	! open time series files and write header lines ===========================!
	! Here [tser_list] is the order of tser files in C41_1
 	if(tser_eta == 1) then
 		tser_list = 1
	...
	if(tser_temp == 1) then ! not yet included
		tser_list = 6
		if(tser_format == 1 .or. tser_format == 3) then
...
! 	if(tser_airp == 1) then ! not yet included
! 		tser_list = 7
! 		if(tser_format == 1 .or. tser_format == 3) then
! 	 		pw_tser = pw_airp_tser_tec
! 			id_tser = id_airp_tser_tec
! 			call open_tser_outfiles_tec(pw_tser,id_tser,tser_list)
! 		end if
! 		if(tser_format == 2 .or. tser_format == 3) then
! 	 		pw_tser = pw_airp_tser_vtk
! 			id_tser = id_airp_tser_vtk
! 			call open_tser_outfiles_vtk(pw_tser,id_tser,tser_list)
! 		end if
! 	end if
