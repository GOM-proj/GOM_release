	...
	if(q_interp_method == 1) then	! Linear interpolation
		...
		do i=1,num_WR_cell
			do t=2,q_data_num(WR_Q_ser_id(i))
				...
				u1 = q_ser_time(t-1,WR_Q_ser_id(i)) - reference_diff_days * 86400.0	! [s],	lower bound time
				u3 = q_ser_time(t  ,WR_Q_ser_id(i)) - reference_diff_days * 86400.0	! [s],	upper bound time
								
				if(u2 >= u1 .and. u2 <= u3) then
					v1 = q_ser_Q(t-1,WR_Q_ser_id(i))		! [m3/s],lower bound Q
					v3 = q_ser_Q(t  ,WR_Q_ser_id(i))		! [m3/s],upper bound Q
