...
subroutine calculate_Q_SS
	...
	if(q_interp_method == 1) then	! Linear interpolation
		...
		do i=1,num_SS_cell
			do t=2,q_data_num(SS_Q_ser_id(i))
				...
				u1 = q_ser_time(t-1,SS_Q_ser_id(i)) - reference_diff_days * 86400.0	! [s],	lower bound time
				u3 = q_ser_time(t  ,SS_Q_ser_id(i)) - reference_diff_days * 86400.0	! [s],	upper bound time
								
				if(u2 >= u1 .and. u2 <= u3) then
					v1 = q_ser_Q(t-1,SS_Q_ser_id(i))		! [m3/s],lower bound Q
					v3 = q_ser_Q(t  ,SS_Q_ser_id(i))		! [m3/s],upper bound Q
				
					v2 = (u2-u1)*(v3-v1)/(u3-u1)+v1	! interpolated Q
					Q_add_SS(i) = v2*SS_portion(i)
					exit
				end if
			end do
		end do
		...
		do i=1,num_SS_cell
			...
			do t=1,q_data_num(SS_Q_ser_id(i))
				difference(t) = q_ser_time(t,SS_Q_ser_id(i)) - u2
			end do
			...
			min_index = min_index - 1						
			shift_num = (min_index + order) - q_data_num(SS_Q_ser_id(i))
			if(shift_num > 0) then
				min_index = min_index - shift_num
			end if
			do ii=0,order
				x(ii+1) = q_ser_time(min_index + ii,SS_Q_ser_id(i))
				y(ii+1) = q_ser_Q(min_index + ii,SS_Q_ser_id(i))
			end do
