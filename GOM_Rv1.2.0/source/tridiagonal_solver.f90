...
subroutine tridiagonal_solver(nmax,n,nc,mat_a,mat_b,mat_c,rhs,solution,gam)
   ...
   real(dp),dimension(nmax,nc),intent(in)  :: rhs 						! we are using 3 columns in solve_momentum_equation.f90
   real(dp),dimension(nmax,nc),intent(out) :: solution 				! we are using 3 columns in solve_momentum_equation.f90
   ...
   if(mat_b(1) == 0.0) then
	...
	! jw, This is the original method
   ...
   do j = n-1, 1, -1
	...
	! jw, This is my modification to the Column-ordering; I don't know which method is faster...
	! foreward substitution ===================================================!
!   denominator = mat_b(1)
!   do i = 1, nc
!      solution(1,i) = rhs(1,i) / denominator
