...
subroutine write_flood_map_vtk
	...
	do i=1,MAXELE
		if(tri_or_quad(i) == 3) then
			write(pw_flood_map_vtk,'(I2, 3I10)') tri_or_quad(i), (nodenum_at_cell(j,i)-1, j=1,tri_or_quad(i)) ! cell number starts from "0" in VTK
		else if(tri_or_quad(i) == 4) then
			write(pw_flood_map_vtk,'(I2, 4I10)') tri_or_quad(i), (nodenum_at_cell(j,i)-1, j=1,tri_or_quad(i)) ! cell number starts from "0" in VTK
		end if
	end do
	write(pw_flood_map_vtk,*)
	...
	do i=1,MAXELE
		if(tri_or_quad(i) == 3) then
			write(pw_flood_map_vtk,'(I2)') 5 ! VTK_TRIANGLES
		else if(tri_or_quad(i) == 4) then
			write(pw_flood_map_vtk,'(I2)') 9 ! VTK_QUAD
		end if
	end do
	write(pw_flood_map_vtk,*)
