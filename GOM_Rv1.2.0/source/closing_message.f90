...
subroutine closing_message
	...
	! define time (clock) related variables
	...
	! Get current time 
	! in the future (F95 and later version), use 'DATE_AND_TIME' subroutine
	call date_and_time(date,time,zone,values)	! date_and_time is standard in Fortran 95 and later version.
	call cpu_time(finish)							! cpu_time is standard in Fortran 95 and later version.
	...
	&						"Total cpu time     = ", finish - start,"seconds" 		! This is the CPU time not the wall clock time
	write(pw_run_log,'(A22,F10.0,A10, A1,I2,A1,I2,A1,I2,A1)') &
	&						"Total elapsed time = ", total_elapsed_time,"seconds", '[',hh,':',mm,':',ss,']' ! This is the real elapsed time
	write(pw_run_log,*) " "
