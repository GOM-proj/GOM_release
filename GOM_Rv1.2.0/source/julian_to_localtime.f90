...
subroutine julian_to_localtime(year, month, day, hour, minute, jday)  
   ...
   real(dp), intent(inout) :: jday
	...
	integer :: year1
   real(dp) :: day_temp = 0.0 						! should be initialized
   real(dp), dimension(12) :: temp_month = 0.0 	! should be initialized
   ...
   if(mod(year1,4) == 0) then
      if(year1 /= 1900) then
         if(month >= 3) then
            ...
            ! Normally, I do not prefer a one-line if statement, but I will keep this here for simplicity.
   ...
   ! jday was elapsed julian_day from the beginning of the year, i.e., it starts from 0 elapsed day.
   ! Thus, when it is converted back to the local date, it should starts from day 1.
   ! That is why I add "1" here.
   ! day = int(day_temp)   
   day = int(day_temp) + 1
   
!   hour = int((day_temp - dfloat(day)) *24.0d0 )
!   minute = ((day_temp - dfloat(int(day_temp))) * 24.0_dp - &
!   &				int((day_temp - dfloat(int(day_temp))) * 24.0_dp)) * 60.0_dp + 0.01_dp
