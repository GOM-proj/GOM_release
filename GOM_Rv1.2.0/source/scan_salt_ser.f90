...
subroutine scan_salt_ser
	...
	if(salt_ser_shape == 1) then
		...
		max_salt_data_num = 1 ! to avoid allocation of 0 length array
		max_salt_ser = num_salt_ser
		
		do i = 1, num_salt_ser
			read(pw_salt_ser,*) i1, i2
			max_salt_data_num = MAX(max_salt_data_num,i2) ! update max_salt_data_num
			do t=1,i2
				read(pw_salt_ser,*)
			end do
		end do
		...
		read(pw_salt_ser,*) i1, i2
		max_salt_data_num = MAX(max_salt_data_num,i2) ! update max_eta_data_num		
	end if
