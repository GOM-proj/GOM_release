...
subroutine holland_storm_surge        
   ...
   real(dp),parameter :: air_density = 1.293d0  ! kg/m^3 
   real(dp),parameter :: gradient_to_surface_wind = 0.95d0 
   real(dp),parameter :: air_p_freestream = 1013.0d0 ! standard air pressure in millibars
...
!*************************************************************************************************
      ...
      if(delta_x_hurric /= 0.0) then 
         hurric_beta = atan(delta_y_hurric/delta_x_hurric) ! -pi/2 to pi/2 
      else 
...
!*************************************************************************************************
      do i = 1, maxnod
         ...
         if(x >= 0.9e+15 .and. y >= 0.9e+15) then ! not actual cells 
            ...
            beta_plus_theta = atan(delta_y_hurric/delta_x_hurric) ! -pi/2 to pi/2 
            if(delta_x_hurric < 0.0 ) then 
         ...
         wind_speed_x_holland_hurricane(i) = gradient_velocity*cos(angle)   ! unit = m/s 
         wind_speed_y_holland_hurricane(i) = gradient_velocity*sin(angle)   ! unit = m/s 

      enddo  ! end of do i = 1, maxnod
!*************************************************************************************************
...
!*************************************************************************************************		
		if(holland_flag == 1) then ! holland model: on
      	do i = 1, num_ob_cell
...
!           set air pressure from hurricane to zero
            air_pressure_holland_hurricane(i) = 0.0d0
      enddo 
	...
	end if   ! if( .not. eof(650) .and. hurric_x_2 < 0.9e+15 )then
