!! ===========================================================================! 
!! GOM is developed by Jungwoo Lee & Jun Lee
!! ===========================================================================! 
!! 
!! Calculate voronoi center of elements
!! This will calculate Voronoi center, and then update (substitute) following variables 
!! which is already calculated with geometric center information:
!! 	x_cell(i), y_cell(i)
!! 	x_face(j), y_face(j), delta_j(j)
!!
subroutine calculate_voronoi_center(xn1_at_face, xn2_at_face, yn1_at_face, yn2_at_face)
   use mod_global_variables 
   use mod_file_definition

   implicit none

   real(dp),dimension(maxface),intent(in) :: xn1_at_face, xn2_at_face, yn1_at_face, yn2_at_face

   integer :: i, j, l, i_node_no, off_element_voronoi_center
   real(dp):: temp_x, temp_y, xi, xj, xk, xm, yi, yj, yk, ym, xij, yij, xjk, yjk, xkl, ykl
   real(dp):: numerator, denominator, x1, x2, x3, x4, y1, y2, y3, y4, ua, ub
   ! real(dp):: angle_theta

	!  variables for voronoi center
   real(dp),dimension(maxele) :: voronoi_center_x, voronoi_center_y
   real(dp),dimension(maxface):: voronoi_face_center_x, voronoi_face_center_y, delta_j_voronoi
   ! End of local variables ==================================================!

	! open file output for voronoi center of elements -------------------------!
   open(pw_voronoi_center,file=id_voronoi_center,form='formatted',status='replace')
   write(pw_voronoi_center,'(a)') 'Title = "Voronoi center of element"'
   ! write(pw_voronoi_center,'(a)') 'Variables = "x", "y", "element_number"'
   write(pw_voronoi_center,'(a)') 'i,     x,     y'
   
	! calculate voronoi center of element -------------------------------------!
   do i = 1, maxele
      if(tri_or_quad(i) == 3)then
         xi = x_node(nodenum_at_cell(1,i))
         yi = y_node(nodenum_at_cell(1,i))
         xj = x_node(nodenum_at_cell(2,i))
         yj = y_node(nodenum_at_cell(2,i))
         xk = x_node(nodenum_at_cell(3,i))
         yk = y_node(nodenum_at_cell(3,i))
      
         xij = 0.5_dp*(xi+xj)
         yij = 0.5_dp*(yi+yj)
         xjk = 0.5_dp*(xj+xk)
         yjk = 0.5_dp*(yj+yk)

         numerator   = (xij-xjk)*(xj-xi) + (yij-yjk)*(yj-yi)
         denominator = (xj -xi) *(yk-yj) - (xk -xj) *(yj-yi)

			! checking the voronoi center
         if(numerator*denominator >= 0.0_dp) then
            off_element_voronoi_center = off_element_voronoi_center + 1
         end if

         if(denominator /= 0.0) then
            voronoi_center_x(i) = xjk + numerator/denominator*(yk-yj)
            voronoi_center_y(i) = yjk - numerator/denominator*(xk-xj)
         else
            write(pw_run_log,*) 'denominator of voronoi is zero at', i
            stop 'calculate_voronoi_center.f90, Error #1'
         end if
      else if(tri_or_quad(i) == 4) then
         xi = x_node(nodenum_at_cell(1,i))
         yi = y_node(nodenum_at_cell(1,i))
         xj = x_node(nodenum_at_cell(2,i))
         yj = y_node(nodenum_at_cell(2,i))
         xk = x_node(nodenum_at_cell(3,i))
         yk = y_node(nodenum_at_cell(3,i))
         xm = x_node(nodenum_at_cell(4,i))
         ym = y_node(nodenum_at_cell(4,i))

         xij = 0.5_dp*(xi+xj)
         yij = 0.5_dp*(yi+yj)
         xjk = 0.5_dp*(xj+xk)
         yjk = 0.5_dp*(yj+yk)
         xkl = 0.5_dp*(xk+xm)
         ykl = 0.5_dp*(yk+ym)

         numerator   = (xij-xjk)*(xj-xi) + (yij-yjk)*(yj-yi)
         denominator = (xj -xi) *(yk-yj) - (xk -xj) *(yj-yi)


         temp_x = 0.0_dp
         temp_y = 0.0_dp
         do l = 1, tri_or_quad(i)
            i_node_no = nodenum_at_cell(l,i)
            temp_x = temp_x + x_node(i_node_no)
            temp_y = temp_y + y_node(i_node_no)
         end do
         voronoi_center_x(i) = temp_x / 4.0_dp
         voronoi_center_y(i) = temp_y / 4.0_dp
      end if
   end do
   
   ! write voronoi center of element
   do i=1,maxele
   	write(pw_voronoi_center,'(I8, 2f18.7)') i, voronoi_center_x(i), voronoi_center_y(i)
   end do
   close(pw_voronoi_center)
   
   ! =========================================================================!
	! calculate orthogonal face center coordinate, 
	! compute angles (orientation of local x) and perpendicular distances for sides
	! with respect to voronoi center
	! =========================================================================!
	open(pw_voronoi_face_center,file=id_voronoi_face_center,form='formatted',status='replace')
   write(pw_voronoi_face_center,'(a)') 'Title = "Voronoi face center of element"'
   write(pw_voronoi_face_center,'(a)') 'Variables = "x", "y", "x_normal_vector", "y_normal_vector", "face number"'

   do j = 1, maxface
      if(adj_cellnum_at_face(2,j) /= 0) then
			! compute delta_j_voronoi() = distance between the voronoi centers of two adjacent elements
         delta_j_voronoi(j) = dsqrt (   &
                 &           (voronoi_center_x(adj_cellnum_at_face(2,j))       &
                 &         -  voronoi_center_x(adj_cellnum_at_face(1,j)))**2   &
                 &         + (voronoi_center_y(adj_cellnum_at_face(2,j))       &
                 &         -  voronoi_center_y(adj_cellnum_at_face(1,j)))**2)
         if(delta_j_voronoi(j) == 0.0_dp) then
            write(pw_run_log,*) 'zero distance between voronoi centers', adj_cellnum_at_face(1,j), adj_cellnum_at_face(2,j)
            stop 'calculate_voronoi_center.f90, Error #2'
         end if

			! calculate x and y unit normal vector of unit normal direction
	      ! angle_theta = datan2(xn1_at_face(j) - xn2_at_face(j), yn2_at_face(j) - yn1_at_face(j))
	      ! cos_theta(j) = dcos(angle_theta)
	      ! sin_theta(j) = dsin(angle_theta)
         ! angle_theta0(j) = datan2(xn1_at_face(j) - xn2_at_face(j), yn2_at_face(j) - yn1_at_face(j))
         ! cos_theta(j) = dcos(angle_theta0(j))	! face normal velocity * cos_theta = true east (u) velocity
         ! sin_theta(j) = dsin(angle_theta0(j))	! face normal velocity * sin_theta = true north (v) velocity

			! calculate voronoi face center
         x1 = voronoi_center_x(adj_cellnum_at_face(1,j))
         y1 = voronoi_center_y(adj_cellnum_at_face(1,j))
         x2 = voronoi_center_x(adj_cellnum_at_face(2,j))
         y2 = voronoi_center_y(adj_cellnum_at_face(2,j))

         x3 = xn2_at_face(j)
         y3 = yn2_at_face(j)
         x4 = xn1_at_face(j)
         y4 = yn1_at_face(j)

         denominator = (y4-y3)*(x2-x1) - (x4-x3)*(y2-y1)

         ua = ((x4-x3)*(y1-y3) - (y4-y3)*(x1-x3)) / denominator
         ub = ((x2-x1)*(y1-y3) - (y2-y1)*(x1-x3)) / denominator
  			
  			! this is the original method
  			! this method does not set the voronoi_face_center point on the face...
         ! voronoi_face_center_x(j) = x1 + ua*(x2-x1)
         ! voronoi_face_center_y(j) = y1 + ub*(y2-y1)
         
         ! this is the new method using the bisection point
         voronoi_face_center_x(j) = x_face(j)
         voronoi_face_center_y(j) = y_face(j)
         

         write(pw_voronoi_face_center,'(I8, 2F18.7)') j, voronoi_face_center_x(j), voronoi_face_center_y(j)
      end if      
   end do
   close(pw_voronoi_face_center)
	! End of Voronoi geometry calculation =====================================!
	
	
	! Update cell center and face center with Voronoi geometry ================!
	! We calculated voroni center at every elements,
	! thus update all cell center with voronoi_center values.
   do i = 1, maxele
      x_cell(i) = voronoi_center_x(i)
      y_cell(i) = voronoi_center_y(i)
   end do
   
   ! We only calculated voronoi_face_center when the face locates between two elements.
   ! Thus, for the boundary faces, we have to use other values.
   do j = 1, maxface
   	if(adj_cellnum_at_face(2,j) /= 0) then
   		! if two elements share this face,
   		! update delta_j(j), x_face(j), and y_face(j) with calculated voronoi values.
   		delta_j(j) = delta_j_voronoi(j)
	      x_face(j) = voronoi_face_center_x(j)
	      y_face(j) = voronoi_face_center_y(j)      
   		
      ! jw's correction to calculate delta_j(j) at boundary face
      ! I can use the following two approaches here, but the second one will not work since "boundary_type_of_face" has not yet been defined.
      ! so, let's just use the first approach even though this will include land boundary also...
		else if(adj_cellnum_at_face(2,j) == 0) then ! this approach will include land boundary also
      ! else if(boundary_type_of_face(j) > 0) then ! while this approach will include open boundary only, and that is why this is better approach
      	! if this is a boundary face, we didn't cacluate voronoi values,
      	! thus, update following values with this approach:
      	! 		delta_j(j):
      	! 			(distance between "cell_center, which is the voronoi center" and the "face_center") * 2 (assuming the ghost cell exist at the same distance apart)
      	! 		x_face(j) & y_face(j):
      	! 			don't need to update since those values are already calculated with geometric center values.
      	
      	! compute delta_j(j) at boundary face:
      	! 		calculate distance between the center of the boundary face and the center of the boundary element,
      	! 		then multiply by 2 (for artificial ghost cell center)
      	delta_j(j) = dsqrt( &
      	&	(x_cell(adj_cellnum_at_face(1,j)) - x_face(j))**2 + &
      	&	(y_cell(adj_cellnum_at_face(1,j)) - y_face(j))**2) &
      	&	* 2.0      	
      end if		
   end do
   ! End of cell center and face center update using Voronoi geometry ========!
   
end subroutine calculate_voronoi_center 
