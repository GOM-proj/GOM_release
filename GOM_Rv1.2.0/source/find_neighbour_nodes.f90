...
subroutine find_neighbour_nodes
	...
	do i=1,maxnod
		do ii=1,maxnod
			if(i == ii) then
				dist(ii) = 1.0e15 ! to exclude self node
			else
		...
		if(min_dist < 5000) then ! if minimum distance is less than 5 km
			hurricane_search_range = 20*1000.0 ! search_radius = 20 km
		else
			hurricane_search_range = min_dist * 4 ! increase search radius regarding minimum distance
		end if
		
		! Now, store the search neighbour nodes information
		k = 0
		do ii=1,maxnod
		...
		hurricane_search_count(i) = k ! total node numbers 
	end do
