...
subroutine write_tec2D_head
   ...
   character(len=15) :: zonetype = 'FEQUADRILATERAL'
	...
	if(IS2D_unit_conv > 0.5_dp) then	! 0.1
	...
	write(pw_tec2D,*) ! change the line		
	...
	! write x coordiante
	do i=1,quotient
		write(pw_tec2D,format1) ((x_node((i-1)*100+j)-xn_min) * IS2D_unit_conv, j=1,100) ! shift origin to zero
	end do
	if(remainder > 0) then
		write(pw_tec2D,format2) ((x_node(quotient*100+j)-xn_min) * IS2D_unit_conv, j=1,remainder) ! shift origin to zero
	end if
	write(pw_tec2D,*) ! put a line

	! write y coordiante
	do i=1,quotient
		write(pw_tec2D,format1) ((y_node((i-1)*100+j)-yn_min) * IS2D_unit_conv, j=1,100) ! shift origin to zero
	end do
	if(remainder > 0) then
		write(pw_tec2D,format2) ((y_node(quotient*100+j)-yn_min) * IS2D_unit_conv, j=1,remainder) ! shift origin to zero
	end if
	write(pw_tec2D,*) ! put a line
	
	! Write variabels at node =================================================!
	! eta at node:
	if(IS2D_variable(1) == 1) then
		do i=1,quotient
			! write(pw_tec2D,'(*(F10.5))') (eta_node((i-1)*100+j), j=1,100)
			write(pw_tec2D,format1) (eta_node((i-1)*100+j), j=1,100)
		end do
		if(remainder > 0) then
			! write(pw_tec2D,'(*(F10.5))') (eta_node(quotient*100+j), j=1,remainder)
	...
	! end of x & y coordinate and variables ===================================!
